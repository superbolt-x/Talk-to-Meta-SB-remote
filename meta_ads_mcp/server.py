...
# NOTE: Meta can spend money and delete ad entities — keep the token gate ON, and
# rely on per-tool ToolAnnotations (read/write/destructive) so Claude prompts
# before write/destructive actions even on this authless connector.
...
logger.info("Meta Ads MCP configured — server_url=%s auth_gate=%s", SERVER_URL or "(unset)", bool(AUTH_TOKEN))
...
def main():
    ...
    Auth: authless (Parker-style). TokenGateMiddleware enforces MCP_AUTH_TOKEN
    (from the connector URL ?access_token=... or an Authorization: Bearer header)
    when set; /health is always open.
    """
    import os
    import uvicorn
